...
def gen_merkle_proof(leaves, pos):
    ...
    # Returns list of hashes that make up the Merkle Proof
        if level_pos % 2 ==0:
            ...
            hashes.append(state[level_pos-1]) #python indexlemesinden dolayı -1
            #sematik merkle treeede durum tam tersi olacak
        state = [hash_internal_node(state[n], state[n+1]) for n in range (0,len(state),2)]
